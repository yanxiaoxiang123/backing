...
项目业务边界清晰，策略注册、统一回测、任务持久化、统一错误体和前端设计变量都是良好基础。后端在 Python 3.12 的 `backing` Conda 环境中验证，当前测试 **406 通过、1 跳过**（前端 43/43）；mootdx 日 K 数据与前端图表已恢复。本轮已修复时间点追溯、回测评测门禁、2026 A 股规则、运行租约、审批归属、CSRF 登录/退出、DSH HTTP 方法与桌面工作台刷新恢复。生产部署仍需处理下方部署与依赖审计事项。

## 本轮修复复核（2026-08-17）

| 风险 | 修复结果 | 验证 |
|---|---|---|
| 研究/回测使用当前数据造成前视 | `agent_runs.as_of` 持久化；行情/因子/财报/新闻按时点截断；缺可得时间的历史数据拒绝；回测加入真实样本外区间 | 后端全量测试；eval lookahead |
| 评测门禁静态恒通过 | lookahead 检查缺 `as_of` 直接失败；回测检查观测日期和 OOS 数据 | `pytest` |
| 2026-07-06 主板 ST 涨跌停规则 | 规则按历史日期计算：生效前 5%，生效后主板 10%，创业板/科创板 20% | `test_paper_rules.py` |
| Agent 重复恢复/多进程竞态 | `execution_owner` + `lease_expires_at` 数据库条件租约，进程内锁仅作快速去重 | 运行时/迁移测试 |
| 审批越权与伪造操作者 | 路由校验 `run_id` 归属，`decided_by` 使用认证主体 | API 路由审阅 |
| 登录/退出被旧 CSRF cookie 阻塞 | 登录端点免 CSRF（仍受限流和 API key 校验）；退出由前端发送 CSRF | `test_auth_session.py` |
| DSH 查询详情 405、无超时 | GET 使用 GET；请求增加 AbortController 超时 | 插件代码审阅 |
| 刷新后工作台丢失 run | `run_id` 写入 URL，刷新自动恢复 REST/SSE；输入区固定在底部 | 前端 typecheck/build/vitest |

仍需另行完成：systemd 前端改静态托管/反向代理、npm 漏洞升级、真实多用户身份模型、TradingAgents 依赖锁定以及生产监控与备份演练。
