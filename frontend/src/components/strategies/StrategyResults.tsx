...
export function StrategyResults({
  ...
  return (
    ...
    >
      {/* Chart */}
      <Card title="信号预览" bodyStyle={{ padding: 0 }} style={{ minHeight: 400 }}>
        {isLoading ? (
        ...
        <Card title="信号历史表现" size="small">
          <Descriptions column={2} size="small" bordered>
            <Descriptions.Item label="买入信号">
              {signalStats.total_buy_signals}
            </Descriptions.Item>
            <Descriptions.Item label="卖出信号">
              {signalStats.total_sell_signals}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="胜率">
              <span
                style={{
                  color: signalStats.win_rate >= 50 ? '#34c759' : '#ff3b30',
                  fontWeight: 600,
                }}
              >
            ...
            <Descriptions.Item label="平均持仓天数">
              {signalStats.avg_holding_days}天
            </Descriptions.Item>
            <Descriptions.Item label="平均每笔收益">
              <span
                style={{
                  color: signalStats.avg_return_per_trade >= 0 ? '#34c759' : '#ff3b30',
                }}
              >
                {signalStats.avg_return_per_trade >= 0 ? '+' : ''}
                {signalStats.avg_return_per_trade}%
              </span>
            </Descriptions.Item>
            <Descriptions.Item label="盈亏比">
              {signalStats.profit_ratio > 0 ? signalStats.profit_ratio.toFixed(2) : '-'}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="最大连赢">
              {signalStats.consecutive_wins}次
            </Descriptions.Item>
            <Descriptions.Item label="最大连亏">
              {signalStats.consecutive_losses}次
            </Descriptions.Item>
          </Descriptions>
        </Card>
      )}
